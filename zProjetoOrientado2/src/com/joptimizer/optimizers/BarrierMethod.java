...
//		}
		...
		if(X0==null){
			...
			if(X0NF!=null){
				if(!Double.isNaN(this.barrierFunction.value(X0NF.toArray()))){
		...
		while (true) {
			...
			ConvexMultivariateRealFunction newObjectiveFunction = new ConvexMultivariateRealFunction() {
				...
				public double[][] hessian(double[] X) {
					DoubleMatrix1D x = F1.make(X);
					DoubleMatrix2D phiHess = F2.make(barrierFunction.hessian(X));
					return getHessF0(x).assign(Mult.mult(tIter)).assign(phiHess, Functions.plus).toArray();
				}
