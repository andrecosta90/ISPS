...
import tecnicas.mvs.libsvm.core.svm_problem;
...
public class svm_train {
	...
	private void do_cross_validation(ArrayList infos) {
		int i;
		int total_correct = 0;
		...
		if (param.svm_type == svm_parameter.EPSILON_SVR
			...
			for (i = 0; i < prob.l; i++) {
				...
				// v = 0.01;
				// }
				// error_mape += Math.sqrt(Math.pow((v-y)/v,2));
				error_mape += Math.abs((v - y) / v);
			}

			// error_mape = Math.sqrt(Math.pow(error_mape, 2));

			if (infos != null) {
				infos.add(total_error / prob.l);
				infos.add(Math.sqrt(total_error / prob.l));
				infos.add(error_mape / prob.l);
			}

			System.out.print("Cross Validation Mean squared error (MSE)= "
					+ total_error / prob.l + "\n");
			System.out
					.print("Cross Validation Root-Mean squared error (RMSE) = "
							+ Math.sqrt(total_error / prob.l) + "\n");
			System.out
					.print("Cross Validation Mean Absolute Percent Error (MAPE) = "
							+ error_mape / prob.l + "\n");
			System.out
	...
	private void ler_problema(String dados) {
	...
	public void executar(String[] argv, String dadosTreino, ArrayList infos)
			throws IOException {
		// TODO Auto-generated method stub
		parse_command_line(argv);
		ler_problema(dadosTreino);
		importantCode(infos);

	}

	private void importantCode(ArrayList infos) throws IOException {
